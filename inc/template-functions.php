...
function logistico_widgets_init() {
	...
	if ( class_exists( 'WooCommerce' ) ) :
		...
		register_sidebar(
			array(
				'name'          => esc_html__( 'Products Sidebar', 'logistico' ),
				'id'            => 'product-sidebar',
				'description'   => esc_html__( 'Add widgets here.', 'logistico' ),
				'before_widget' => '<section id="%1$s" class="widget %2$s">',
				'after_widget'  => '</section>',
...
if ( ! function_exists( 'logistico_fonts_url' ) ) :
	...
	function logistico_fonts_url() {
		...
		if ( 'off' !== _x( 'on', 'Bricolage Grotesque font: on or off', 'logistico' ) ) {
			$fonts[] = 'Bricolage Grotesque:700';
		}

		/* translators: If there are characters in your language that are not supported by Muli, translate this to 'off'. Do not translate into your own language. */
		if ( 'off' !== _x( 'on', 'Gothic A1 font: on or off', 'logistico' ) ) {
			$fonts[] = 'Gothic A1:400,500,700';
		}
...
if ( !function_exists( 'logistico_content_class' ) ) {
	...
	function logistico_content_class() {
		...
		$product_default_sidebar = get_theme_mod( 'logistico_product_page_sidebar', 'right_sidebar' );
		$product_singlet_sidebar = get_theme_mod( 'logistico_single_product_sidebar', 'right_sidebar' );

		if ( is_woocommerce_active() && is_product() ) {
			$content_class = logistico_content_wrap_calc( $product_singlet_sidebar );
		} elseif ( is_single() ) {
			...
			$content_class = logistico_content_wrap_calc( $product_default_sidebar );
		} else {
...
if ( !function_exists( 'logistico_sidebar_class' ) ) {
	...
	function logistico_sidebar_class() {
		...
		if ( is_woocommerce_active() && is_product() ) {
			$sidebar_class = logistico_sidebar_wrap_calc( $product_singlet_sidebar );
		} elseif ( is_single() ) {
			...
			$sidebar_class = logistico_sidebar_wrap_calc( $product_default_sidebar );
		} else {
...
function logistico_woo_wrapper_start(){
	echo '<div id="primary" class="' . esc_attr( logistico_content_class() ) . '"><main id="main" class="site-main" role="main">';
}

function logistico_woo_hooks(){
	remove_action( 'woocommerce_before_main_content', 'woocommerce_output_content_wrapper', 10 );
	add_action('woocommerce_before_main_content', 'logistico_woo_wrapper_start', 10);

	$related_product = get_theme_mod( 'logistico_related_product_option', 'show' );
	if( $related_product === 'hide' ) {
		remove_action( 'woocommerce_after_single_product_summary', 'woocommerce_output_related_products', 20 );
	}
}
add_action('wp_head', 'logistico_woo_hooks');
